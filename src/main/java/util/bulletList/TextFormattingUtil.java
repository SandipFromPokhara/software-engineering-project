...
public class TextFormattingUtil {
    ...
    public static void toggleList(TextArea textArea, Button button, ListFormattingStrategy strategy) {
        ...
        // If text area is empty, insert default prefix
        ...
        // Check if all lines already have this format
        ...
        // Apply or remove format using the strategy
    ...
    // ONE method replaces handleBulletListEnter() and handleNumberedListEnter()
    ...
    private static boolean handleListEnter(TextArea textArea, KeyEvent event,
        ...
        // Only handle if the current line has this strategy's format
        ...
        if (contentAfter.trim().isEmpty()) {
            // Empty list item — remove the format prefix entirely
            String before = text.substring(0, lineStart);
            String after = lineEnd < text.length() ? text.substring(lineEnd) : "";
            textArea.setText(before + after);
            textArea.positionCaret(lineStart);
        } else {
            // Get next line's prefix (number + 1 for numbered, same bullet for bullets)
    ...
    // Get the next line's prefix (e.g. "• " or "3. ")
