# Updated Database Architecture – NoteVault
...
| notebook_id   | INT (FK)      | NO       | Parent notebook; FK references notebooks.id |
| language_code | VARCHAR (6)   | NO       | E.g.; EN, FI, NP                            |
...
- UNIQUE INDEX: `(notebook_id, language_code)` → Prevents duplicate languages per notebook.

- FOREIGN KEY: `notebook_id` → `notebooks.id` ON DELETE CASCADE.
...
| notebook_id | INT (FK)      | NO       | References notebooks.id. Deleting the notebook cascades deletion of notes |
| createdAt   | DATETIME      | NO       | DEFAULT CURRENT_TIMESTAMP                                                 |
| updatedAt   | DATETIME      | YES      | ON UPDATE CURRENT_TIMESTAMP                                               |
...
|---------------|---------------|----------|-------------------------------------|
| id            | INT (PK)      | NO       | Unique identifier                   |
| note_id       | INT (FK)      | NO       | Parent note; FK references notes.id |
| language_code | VARCHAR (6)   | NO       | E.g.; EN, FI, NP                    |
...
- UNIQUEINDEX: `(note_id, language_code)` → One translation per language per note.
...
```
User (0..N) ────── (1) Notebook
Notebook (1) ───── (1..N) Notebook_Translation
Notebook (0..N) ── (1) Note
Note (1) ───────── (1..N) Note_Translation
Note (0..N) ────── (0..N) Tag
```
...
- Deleting a Notebook → Deletes associated notes and its notebook translations.

- Deleting a Note → Deletes entries in note_tags and its note translations, tags remain.
...
## Notes
This schema represents the updated design of NoteVault.
...
- Note sharing.
- 
- Language Expansion: Automated translation services.
